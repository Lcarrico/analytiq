...
export const api = {
  ...
  patchGrid: (sid, body) => req(`/sessions/${sid}/dashboard-spec/grid`, { method: 'PATCH', body: JSON.stringify(body) }),
  hydrateSession: (sid) => req(`/sessions/${sid}/hydrate`),
  chatPatch: (sid, message) => req(`/sessions/${sid}/chat-patch`, { method: 'POST', body: JSON.stringify({ message }) }),
  applyPatch: (sid, ops) => req(`/sessions/${sid}/dashboard-patch`, { method: 'POST', body: JSON.stringify({ ops }) }),
  restoreSpec: (sid, version) => req(`/sessions/${sid}/dashboard-spec/restore`, { method: 'POST', body: JSON.stringify({ version }) }),
