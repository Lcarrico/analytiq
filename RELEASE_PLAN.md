...
- [x] **R32S2E3-US1** — field picker (3-panel, cardinality warning banner, live preview `100-row cap · Nms` — `DEP:` bounded read-only preview endpoint, seeded deterministic) + join paths (SAFE/BLOCKED/FAN-OUT pills, bridge-table rec CTA) + derived-tables editor (dark SQL validate, schedule, tags, dry run, FRESH/STALE list) · [was R25S2E3]
  - Agent Notes (R32S2, 2026-07-05): preview endpoint is seeded-deterministic (no warehouse), 100-row cap, name-keyed cardinality heuristics for series estimates. Pre-agg speedup/cost are demo-derived (labeled in-UI); materialize/dismiss persistence owned by R36S1. Join manager: builder emits validated n:1/left joins only — BLOCKED m:n renders conditionally; FAN-OUT path made real via sim fk `null_pct 4.1` (manifests now carry null_pct). Explores are 1:1 with cubes. S05 panels (evolution proposals R10S2E5, evidence triage R10S2E6, schema compare R11S2E4) rehomed onto the overview, citations removed, ledger pruned.
- [x] R32S2 sprint regression recorded — backend 440/440 · UI 153/153 (2026-07-05)
### Release R32 close-out
- [x] Release regression + zero-key boot recorded · PAR-1 flips: Governance ×4 · Governance Lineage ×3 · Semantic ×9 — **R32 CLOSED 2026-07-05**: BE 440/440 · UI 153/153 · zero-key boot all 8 services local · client shell 200
...
- [x] **R34S1E2-US1** — landing rebuild (dark hero + populated live-build preview, BI comparison, value props w/ icon chips, use-cases 3×2, trust strip, CTA band w/ mono terminal); `r29s1_marketing.spec.js` → `r34s1_marketing.spec.js` rename-on-touch (done in E1, this story's first actual touch of the file) · [was R29S1E2] ✅ 2026-07-06 (Hero/HeroPreview/BiComparison/ValueProps/UseCases/TrustStrip/CtaBand in Marketing.jsx; spec updated to match new copy, not executed — see environment note; backend 467/469, same 2 pre-existing unrelated failures)
- [x] **R34S1E3-US1** — product page (header, sticky 5-step stepper, 5 alternating stage sections, dark CTA) · [was R29S1E3] ✅ 2026-07-06 (new MarketingProduct.jsx + /product route; spec extended, not executed — see environment note; backend 466/469, 3rd failure confirmed flaky/order-independent — passes in isolation, no backend files touched)
- [x] **R34S1E4-US1** — pricing restyle (header + Monthly/Annual −20% toggle, /mo mono prices, descriptors, CTAs per card, floating MOST POPULAR pill + 2px glow, dark Enterprise card, comparison table, FAQ accordion, footer). **AC: plan data still matches ch02 exactly — `r30s1_pricing_data.spec.js` stays green (regression lock)** · [was R29S1E4] ✅ 2026-07-06 (PLANS array untouched, presentation-only PLAN_META added alongside it; caught+fixed a flex-gap-vs-literal-space bug that would have broken the exact-text lock, before running anything; r30s1_pricing_data + r34s1_marketing specs updated/extended, not executed — see environment note; backend 467/469, same 2 pre-existing failures)
...
- [x] **R34S2E2-US1** — templates gallery (250px filter rail, grid header w/ count + search, 10 template cards w/ SVG art) · [was R29S2E2] ✅ 2026-07-06 (new MarketingTemplates.jsx + /templates route; real client-side filtering via useMemo; preserved a pre-existing mockup inconsistency — "Margin Variance" has no matching Type checkbox — rather than inventing one; spec extended, not executed; backend 466/469, same 2 pre-existing + 1 flaky)
- [x] **R34S2E3-US1** — security page (compliance pills, sticky jump nav, 8 tinted section cards) · [was R29S2E3] ✅ 2026-07-06 (new MarketingSecurity.jsx + /security route; jump-nav links fully functional, active-while-scrolling highlight left static rather than building IntersectionObserver scroll-spy for a cosmetic-only effect — flagged in tracked_changes.md; spec extended, not executed; backend 466/469, same known failures)
- [x] **R34S2E4-US1** — docs page (slim docs nav w/ ⌘K, nav tree, Quickstart article w/ terminal block + amber callout, on-this-page rail) · [was R29S2E4] ✅ 2026-07-07 (new MarketingDocs.jsx + /docs route; own distinct nav, no shared footer — confirmed with user, mockup draws no footer on this page at all; non-Quickstart nav-tree items render as inert placeholders per App.jsx's PLACEHOLDERS precedent, not fabricated articles; spec extended, not executed; backend 466/469, same known failures)
- [x] R34S2 sprint regression recorded — backend 466/469 (2 confirmed pre-existing + 1 confirmed flaky, all unrelated to this work); UI not executed this engagement — see environment note
### Release R34 close-out
- [x] Release regression + zero-key boot recorded ✅ 2026-07-07 — backend 466/469 (2 confirmed pre-existing + 1 confirmed flaky, all unrelated to this work — see per-story notes above); zero-key boot verified: shell `/` → 200, `/api/platform/status` all 8 services `local`. UI (Playwright) not executed this engagement — pre-existing environment gap (`@sparticuz/chromium` extracts a Linux binary on this native Windows machine; no Chromium browser installed at all), documented in `PROGRESS.md` and `lucaContext/tracked_changes.md`, deliberately left unfixed as out of scope for a marketing-content release. PAR-1 flips: Marketing ×7 (all 7 pages now live at their designed routes with real content)
...
- [x] **R35S2E1-US1** — source detail tabs · [was R24S2E1]
- [x] **R35S2E2-US1** — table detail (profile, columns, PII flags) · [was R24S2E2]
