...
Locator: getByText('Available', { exact: true }).first()
Expected: visible
Timeout: 10000ms
...
Call log:
  ...
  - waiting for getByText('Available', { exact: true }).first()
...
```ts
  ...
  118 |   const addBtn = page.getByText('+ Add new data source');
  119 |   if (await addBtn.count()) await addBtn.click();
  120 |   const legacyBadge = page.getByText('Available', { exact: true }).first();
> 121 |   await expect(legacyBadge).toBeVisible();
      |                             ^ Error: expect(locator).toBeVisible() failed
  122 |   expect(await css(legacyBadge, 'borderRadius')).toBe('999px'); // was 4px
  123 |   expect(await css(legacyBadge, 'fontFamily')).toContain('Mono');
  124 | });
  125 | 
```