...
- Name: r21s1_kit.spec.js >> parity screenshot pair captured for the kit
- Location: tests/ui/r21s1_kit.spec.js:102:5
...
> 105 |   await expect(page.getByTestId('kit-gallery')).toBeVisible();
  ...
  117 |   await expect(legacyBadge).toBeVisible();
  118 |   expect(await css(legacyBadge, 'borderRadius')).toBe('999px'); // was 4px
  119 |   expect(await css(legacyBadge, 'fontFamily')).toContain('Mono');
  120 | });
  121 | 
```